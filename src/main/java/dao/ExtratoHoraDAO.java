...
public class ExtratoHoraDAO extends BaseDAO {
    ...
    public int qtdHoraAprovada(int idUser) {
        try {
            String sql = "SELECT COUNT(*) FROM Extrato_Hora eh inner join usuario user on eh.Id_Usuario = user.Id  WHERE  user.Id_Tipo_Usuario = 1  AND user.Id = "
                    + idUser + " AND Id_Etapa_Extrato =" + EtapaExtrato.APROVADA.ordinal();
    ...
    public int qtdHoraReprovada(int idUser) {
        try {
            String sql = "SELECT COUNT(*) FROM Extrato_Hora eh inner join usuario user on eh.Id_Usuario = user.Id  WHERE  user.Id_Tipo_Usuario = 1 AND user.Id = "
                    + idUser + " AND Id_Etapa_Extrato =" + EtapaExtrato.REPROVADA.ordinal();
            return executeCount(sql);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
    ...
    public int qtdHoraCrAprovada() {
        try {
            String sql = "SELECT COUNT(*) FROM extrato_hora WHERE  Id_Cr AND Id_Etapa_Extrato ="
                    + EtapaExtrato.APROVADA.ordinal();
            return executeCount(sql);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
    ...
    public int qtdHoraCrReprovada() {
        try {
            String sql = "SELECT COUNT(*) FROM extrato_hora WHERE  Id_Cr AND Id_Etapa_Extrato ="
                    + EtapaExtrato.REPROVADA.ordinal();
            return executeCount(sql);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
